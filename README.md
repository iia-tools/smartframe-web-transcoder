...
2. 이 페이지는 같은 LAN의 SmartFrame Hub에서 원본을 Range 요청으로 직접 읽습니다.
3. 브라우저가 긴 변 1280px, H.264/AAC MP4로 변환합니다.
4. 결과를 4MB 조각으로 SmartFrame Hub에 직접 올립니다.
5. 프레임은 H.264 트랙과 해상도를 검사한 뒤 `스마트프레임 캐시 / 브라우저 변환`에 사본을 저장합니다.

GitHub Pages와 별도 서버는 영상, PIN, 계정 정보에 접근하지 않습니다. 실행 권한은 URL fragment에 실리므로 GitHub 서버 요청에도 포함되지 않습니다.
